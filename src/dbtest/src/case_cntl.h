...
class TxnSql {
 ...
 public:
  TxnSql(const int sql_id, const int txn_id, const std::string& sql, const std::string& test_case_type)
      : txn_id_(txn_id), sql_id_(sql_id), sql_(sql), test_case_type_(test_case_type){};

  int TxnId() { return txn_id_; };
  int SqlId() { return sql_id_; };
  std::string Sql() { return sql_; };
  std::string TestCaseType() { return test_case_type_; };
  std::string toString() { return "Q" + std::to_string(sql_id_) + "-T" + std::to_string(txn_id_) + "-" + sql_; }
};
...
class CaseReader {
 ...
 public:
  ...
  void AddTestSequence(TestSequence test_sequence) { test_sequence_list_.push_back(test_sequence); };
  void AddTestResultSet(TestResultSet test_result_set) { test_result_set_list_.push_back(test_result_set); };
...
class ResultHandler {
 public:
  bool IsSqlExpectedResult(std::vector<std::string> cur_result, std::vector<std::string> expected_result, const int sql_id,
...
class Outputter {
 public:
  ...
  bool PrintAndWriteTxnSqlResult(std::vector<std::string> cur_result,
                                 std::vector<std::unordered_map<int, std::vector<std::string>>> expected_result_set_list, const int sql_id,
                                 const std::string& sql, const int session_id, const std::string& test_process_file);
