 ...
 public:
  bool InitDBConnector(std::string& user, std::string& passwd, std::string& db_type, int conn_pool_size) {
    for (int i = 0; i < (int)conn_pool_size; i++) {
      ...
      // connect
      ret = SQLConnect(m_hDatabaseConnection, (SQLCHAR*)db_type.c_str(), SQL_NTS, (SQLCHAR*)user.c_str(), SQL_NTS, (SQLCHAR*)passwd.c_str(),
                       SQL_NTS);
  ...
  bool ExecReadSql2Int(int sql_id, const std::string& sql, TestResultSet& test_result_set,
                       std::unordered_map<int, std::vector<std::string>>& cur_result_set, int conn_id, int param_num,
  ...
  std::string SqlExecuteErr(int session_id, int sql_id, const std::string& sql, std::string handle_type, SQLHANDLE& handle, SQLRETURN ret,
                            std::string test_process_file = "");

  bool SQLEndTnx(std::string opt, int session_id, int sql_id, TestResultSet& test_result_set, const std::string& db_type,
                 std::string test_process_file = "");
  bool SQLStartTxn(int session_id, int sql_id, std::string test_process_file = "");
  bool SetIsolationLevel(SQLHDBC m_hDatabaseConnection, std::string opt, int conn_id, const std::string& db_type,
                         std::string test_process_file = "");
