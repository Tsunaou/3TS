...
bool MultiThreadExecution(std::vector<TxnSql>& txn_sql_list, TestSequence& test_sequence, TestResultSet& test_result_set,
                          ...
                          std::unordered_map<int, std::vector<std::string>>& cur_result_set, int sleeptime) {
    ...
    if (index_read != sql.npos || index_read_1 != sql.npos) {
      ...
      if (FLAGS_db_type != "crdb" && FLAGS_db_type != "ob") {
        if (FLAGS_db_type == "tidb") {
          ...
          //     if (!db_connector.ExecWriteSql(sql_id, "START TRANSACTION WITH CONSISTENT SNAPSHOT;", test_result_set, txn_id,
          //     test_process_file)) {
          //     // if (!db_connector.ExecWriteSql(sql_id, "BEGIN OPTIMISTIC;", test_result_set, txn_id, test_process_file)) {
          //         goto jump;
          //     }
...
int main(int argc, char* argv[]) {
  ...
  std::string test_path = FLAGS_case_dir;
  if (!case_reader.InitTestSequenceAndTestResultSetList(test_path, FLAGS_do_test_list, FLAGS_db_type)) {
