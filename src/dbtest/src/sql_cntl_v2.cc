...
std::string DBConnector::SqlExecuteErr(int session_id, int sql_id, const std::string& sql, std::string handle_type, SQLHANDLE& handle,
                                       SQLRETURN ret, std::string test_process_file) {
  ...
  if (ret == SQL_SUCCESS || ret == SQL_SUCCESS_WITH_INFO) {
    ...
    if (sql_id != 1024 && index_not_exist == err_info.npos && index_crdb_rollback == sql.npos) {
      ...
      std::cout << blank + "Q" + std::to_string(sql_id) + "-T" + std::to_string(session_id) + " failed reason: " << err_info
                << " errcode: " << SQLState << std::endl;
      test_process << blank + "Q" + std::to_string(sql_id) + "-T" + std::to_string(session_id) + " failed reason: " << err_info
                   << " errcode: " << SQLState << std::endl;
      // if (!test_process) {
      //     test_process << blank + "execute sql: '" + sql + "' failed, reason: " << ErrInfo << " errcode: " << SQLState << std::endl;
      // }
      std::string output_time_info =
          blank + "Q" + std::to_string(sql_id) + "-T" + std::to_string(session_id) + " failed at: " + get_current_time();
      ...
      //     test_process << output_time_info << std::endl;
      // }
      this->txn_finished_[session_id] = true;
      return err_info;
    } else {
      return "";
    }
...
bool DBConnector::ExecWriteSql(int sql_id, const std::string& sql, TestResultSet& test_result_set, int session_id,
                               std::string test_process_file) {
  SQLRETURN ret;
  ...
  SQLFreeStmt(m_hStatement, SQL_DROP);
  SQLFreeStmt(m_hStatement, SQL_UNBIND);
  if ((sql.find("UPDATE") != sql.npos || sql.find("INSERT") != sql.npos || sql.find("SELECT") != sql.npos) && row_count == 0 && sql_id != 0 &&
      sql_id != 1024) {
    std::string blank(blank_base * (session_id - 1), ' ');
    std::string output_time_info =
        blank + "Q" + std::to_string(sql_id) + "-T" + std::to_string(session_id) + " failed at: " + get_current_time();
  ...
  // get error info
    ...
    if (sql_id != 1024 && sql_id != 0) {
      std::string blank(blank_base * (session_id - 1), ' ');
      std::string output_time_info =
          blank + "Q" + std::to_string(sql_id) + "-T" + std::to_string(session_id) + " finished at: " + get_current_time();
      std::cout << output_time_info << std::endl;
      std::ofstream test_process(test_process_file, std::ios::app);
      test_process << output_time_info << std::endl;
    }
...
bool DBConnector::ExecReadSql2Int(int sql_id, const std::string& sql, TestResultSet& test_result_set,
                                  std::unordered_map<int, std::vector<std::string>>& cur_result_set, int session_id, int param_num,
                                  std::string test_process_file) {
  SQLRETURN ret;
  SQLHSTMT m_hStatement;
  SQLHDBC m_hDatabaseConnection = DBConnector::conn_pool_[session_id - 1];
...
bool DBConnector::SQLEndTnx(std::string opt, int session_id, int sql_id, TestResultSet& test_result_set, const std::string& db_type,
                            std::string test_process_file) {
  if (sql_id != 1024) {
    std::string blank(blank_base * (session_id - 1), ' ');
...
bool DBConnector::SetIsolationLevel(SQLHDBC m_hDatabaseConnection, std::string opt, int session_id, const std::string& db_type,
                                    std::string test_process_file) {
  // oracle mode
  if (db_type != "oracle" && db_type != "ob") {
    ...
    std::string err_info_stmt =
        DBConnector::SqlExecuteErr(session_id, 1024, "set isolation", "dbc", m_hDatabaseConnection, ret, test_process_file);
    if (!err_info_stmt.empty()) {
      return false;
    }
  } else {
