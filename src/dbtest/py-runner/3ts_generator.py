...
def init_table(file_name, sql_count, txn_count, table_num, db_type, test_type):
    ...
    with open(file_name, "a+") as file_test:
        for i in range(1, table_num + 1):
            drop_sql = str(sql_count) + "-" + str(txn_count) + \
                "-" + "DROP TABLE IF EXISTS t" + str(i) + ";\n"
...
def execute_sql(IsPredicate, file_name, sql_count, txn_count, op_num, op, data_num, txn, data_value, data_op_list):
    ...
    if op == "W":
        if IsPredicate:
            sql_count = write_data(file_name, sql_count, txn_count, op_num * 2 + 1, data_num, txn, data_value,
                                   data_op_list)
        else:
            sql_count = write_data(
                file_name, sql_count, txn_count, op_num, data_num, txn, data_value, data_op_list)
    elif op == "R":
        if IsPredicate:
            sql_count = read_data(
                file_name, sql_count, txn_count, op_num * 2 + 1, data_num, txn, data_op_list)
        else:
            sql_count = read_data(
                file_name, sql_count, txn_count, op_num, data_num, txn, data_op_list)
    elif op == "P":
        sql_count = read_data_predicate(
            file_name, sql_count, txn_count, op_num, data_num, txn, data_op_list)
    # I and D only works for single test_type now
    elif op == "I":
        sql_count = insert_data(file_name, sql_count, txn_count,
                                op_num * 2 + 1, 1, 1, data_num, exist, data_value)
    elif op == "D":
        sql_count = delete_data(file_name, sql_count, txn_count,
                                op_num * 2 + 1, 1, data_num, exist, txn, data_op_list)
    elif op == "A":
...
def insert_data(file_name, sql_count, txn_count, cur_count, partition_num, insert_table, data_num, exist,
    ...
    with open(file_name, "a+") as file_test:
        try:
            ...
            if data_num == 2:
                file_test.write("data" + " k=" + str(cur_count) +
                                " already exists, can't insert")
                print("data" + " k=" + str(cur_count) +
                      " already exists, can't insert")
            else:
                file_test.write("data" + " value1=" +
                                str(cur_count) + " already exists, can't insert")
                print("data" + " value1=" + str(cur_count) +
                      " already exists, can't insert")
    return sql_count
            ...
            if txn[txn_count].end_ts != max_time:
                ...
                if data_num == 2:
                    delete_sql = str(sql_count) + "-" + str(txn_count) + "-" + "DELETE FROM t" + \
                        str(delete_table) + " WHERE k=" + \
                        str(cur_count) + ";\n"
                else:
                    delete_sql = str(sql_count) + "-" + str(txn_count) + "-" + "DELETE FROM t" + \
                        str(delete_table) + " WHERE value1=" + \
                        str(cur_count) + ";\n"
...
def write_data(file_name, sql_count, txn_count, op_num, data_num, txn, data_value, data_op_list):
    with open(file_name, "a+") as file_test:
        try:
            ...
            file_test.write(
                "data doesn't exist or the transaction has ended and cannot be updated")
    ...
    with open(file_name, "a+") as file_test:
        try:
            if txn[txn_count].end_ts != max_time:
                ...
                abort_sql = str(sql_count) + "-" + \
                    str(txn_count) + "-" + "ROLLBACK;\n"
                file_test.write(abort_sql)
        except OptionException:
            file_test.write("transaction" + str(txn_count) +
                            " ended and can't be rolled back again")
            print("transaction" + str(txn_count) +
                  " ended and can't be rolled back again")
    return sql_count
    ...
    with open(file_name, "a+") as file_test:
        try:
            if txn[txn_count].end_ts != max_time:
                ...
                txn[txn_count].end_ts = sql_count
                commit_sql = str(sql_count) + "-" + \
                    str(txn_count) + "-" + "COMMIT;\n"
                file_test.write(commit_sql)
        except OptionException:
            file_test.write("transaction" + str(txn_count) +
                            " ended and can't be committed again")
            print("transaction" + str(txn_count) +
                  " ended and can't be committed again")
    return sql_count
...
def execute_check(file_name, sql_count, txn_count, data_num, table_num):
    with open(file_name, "a+") as file_test:
        ...
        for i in range(1, table_num + 1):
            if data_num == 2:
                read_sql = str(sql_count) + "-" + str(txn_count) + \
...
def execute_first(IsPredicate, num, ops, file_name, sql_count, txn_count, data_num, txn, data_value, data_op_list):
    for i in range(num):
        ...
        if last_op_num == op_num and last_op_type == op1:
            return sql_count
        sql_count = execute_sql(IsPredicate, file_name, sql_count, txn_count, op_num, op1, data_num, txn, data_value,
                                data_op_list)
        sql_count += 1
        txn_count += 1
        if txn_count == num + 1:
...
def execute_second(IsPredicate, num, ops, need_ac, file_name, sql_count, txn_count, data_num, txn, data_value,
                   data_op_list):
    for i in range(num):
        if ops[i][1] == "C" or ops[i][1] == "A":
            op2 = ops[i][2]
            ...
            op2 = ops[i][1]
            op_num = int(ops[i][2:])
        sql_count = execute_sql(IsPredicate, file_name, sql_count, txn_count, op_num, op2, data_num, txn, data_value,
                                data_op_list)
        sql_count += 1
...
def execute_txn(IsPredicate, num, ops, need_ac, file_name, sql_count, data_num, txn, data_value, wait_op_list,
                data_op_list):
    for i in range(num):
        op1 = ops[i][0]
        if ops[i][1] == "C" or ops[i][1] == "A":
        ...
        if i == 0:
            sql_count = execute_sql(IsPredicate, file_name, sql_count, i + 1, op_num, op1, data_num, txn, data_value,
                                    data_op_list)
            sql_count += 1
            ...
            if last_op_num == op_num:
                ...
                for data in wait_op_list:
                    sql_count = execute_sql(IsPredicate, file_name, sql_count, data.txn_num, data.op_num, data.op_type,
                                            data_num, txn, data_value, data_op_list)
                    sql_count += 1
                ...
                sql_count = execute_sql(IsPredicate, file_name, sql_count, i + 1, op_num, op1, data_num, txn,
                                        data_value, data_op_list)
                sql_count += 1

                if need_ac[i + 1] != "NO":
                    sql_count = execute_sql(IsPredicate, file_name, sql_count, i + 1, op_num, need_ac[i + 1], data_num,
                                            txn, data_value, data_op_list)
                    sql_count += 1

                sql_count = execute_sql(IsPredicate, file_name, sql_count, next_txn_count, op_num, op2, data_num, txn,
                                        data_value, data_op_list)
                sql_count += 1
            else:
                sql_count = execute_sql(IsPredicate, file_name, sql_count, i + 1, op_num, op1, data_num, txn,
                ...
                if len(wait_op_list) == 0:
                    if need_ac[i + 1] != "NO":
                        sql_count = execute_sql(IsPredicate, file_name, sql_count, i + 1, op_num, need_ac[i + 1],
                                                data_num, txn, data_value, data_op_list)
                        sql_count += 1
                wait_op_list.append(Wait_Operation(
                    op2, next_txn_count, op_num))

    for data in wait_op_list:
        sql_count = execute_sql(IsPredicate, file_name, sql_count, data.txn_num, data.op_num, data.op_type, data_num,
                                txn, data_value, data_op_list)
        sql_count += 1

        if need_ac[data.txn_num] != "NO":
            sql_count = execute_sql(IsPredicate, file_name, sql_count, data.txn_num, data.op_num, need_ac[data.txn_num],
                                    data_num, txn, data_value, data_op_list)
            sql_count += 1
    wait_op_list = []
...
def write_description(file_name, txn_num, op_num, data_num):
    with open(file_name, "w+") as file_test:
        ...
        description += "T1\n"
        description += "# Parameters: #column=2 #txn=" + str(txn_num) + " #operations=" + str(
            op_num) + " #variable=" + str(data_num) + "\n"
...
for popg in lines:
    ...
    # preparation
    data_num = init_table(file_name, sql_count, txn_count,
                          table_num, db_type, test_type)
    ...
    for i in range(len(ops)):
        ...
        if IsPredicate and not visit[op_num]:
            ...
            if ops[i][0] == "R" or ops[i][0] == "P":
                if ops[i].find("I") == -1:
                    insert_data(file_name, sql_count, txn_count, 2 * op_num + 1, partition_num, insert_table, data_num,
                                exist, data_value)
    ...
    need_ac = ["NO"] * (num + 1)
    sql_count += 1
    sql_count = execute_txn(IsPredicate, num, ops, need_ac, file_name, sql_count, data_num, txn, data_value,
                            wait_op_list, data_op_list)
...
def create_output_directory(dir: str):
    if not os.path.exists(dir):
        os.mkdir(dir)


class Generator:
    def __init__(self, deploy, dbtype, case_folder) -> None:
        super().__init__()
        self.deploy = deploy
        self.dbtype = dbtype
        self.predicate = False
        self.output = case_folder

        create_output_directory(self.output)

    def gen(self, pop_graph: str):
        if any(op in pop_graph for op in ['I', 'D', 'P']):
            self.predicate = True
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--deploy", default='single', help="数据库架构")
    parser.add_argument("-t", "--dbtype", default='pg', help="数据库种类")
    parser.add_argument(
        "-f", "--input", default='do_test_list.txt', help="输入文件")
    parser.add_argument("-o", "--output", default='t/test_case', help="输出文件夹")
    args = parser.parse_args()

    gen = Generator(deploy=args.deploy, dbtype=args.dbtype,
                    case_folder=args.output)

    do_test_list = args.input
    with open(do_test_list, 'r') as f:
        lines = f.readlines()

    for line in lines:
        pop_graph = line.strip('\n').replace(' ', '')
        if not pop_graph or pop_graph.startswith('#'):
            continue
